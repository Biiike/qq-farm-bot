...
- `LOG_LEVEL=warn`（仅输出 warn/error，显著减少日志量）
- `LOG_TO_FILE=0`（关闭本地文件日志 `logs/*.log`）
- `DASHBOARD_MAX_LOGS=0`（关闭进程内日志缓冲，进一步降低内存占用）
- `QR_LOGIN=1`（QQ 模式下启用扫码登录；容器日志会输出可打开的二维码链接）
...
点击 `实时应用巡查间隔` 后会立即写入运行中机器人参数，无需重启进程。
